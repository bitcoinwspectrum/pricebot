...
token = 'NDI2Mzc1NDg0MzI3MTk4NzMx.DcOb3Q.40iwI_oCLX8cIWuH2j49SK8fVqA'
...
async def on_message(message):
    ...
    if message.content.startswith('!balance 0conf'): #0conf balance
        ...
        msg = '{0.author.mention}, you have %f bws, including 0-confs.'.format(message)%balance
        return await client.send_message(message.channel,msg)
    ...
    if message.content.startswith('!balance'):
        ...
        msg = '{0.author.mention}, you have %f bws ($%.2f).'.format(message)%(balance,price)
        return await client.send_message(message.channel,msg)
    ...
    if message.content.startswith('!tip '):
        ...
        try:
            ...
            return await client.send_message(message.channel,"{0.author.mention} has tipped %s %f bws ($%.2f).".format(message)%(toTipMention,amount,price))
        except ValueError:
    ...
    if message.content.startswith('!rain '):
        ...
        if amount < 0.01:
            return await client.send_message(message.channel,"{0.author.mention}, the amount must be bigger than 0.01 bws.".format(message))
        #catching errors again
        try:
            amount = float(amount)
        except ValueError:
        ...
        try:
            ...
            return await client.send_message(message.channel,"{0.author.mention} has tipped %f bws to everyone on this server!".format(message)%eachtip)
        except ValueError:
            return await client.send_message(message.channel,"{0.author.mention}, insufficient balance.".format(message))
